...
ggp <- ggplot(df,aes(x=gap.size,y=obsd,ymin=ci.low,ymax=ci.hi,color=sim,shape=sim,group=sim)) +
geom_hline(aes(yintercept=val),linetype=5) +
geom_linerange(size=0.5) +
geom_point(size=2) +
geom_line() +
#theme_bw()  +
#scale_colour_manual(name = "Scenario",values=cbbPalette[c(3,5,6)]) +
scale_colour_manual(name = "Scenario",values=c('firebrick','dodgerblue','seagreen'),labels=c('Alt. 2','Alt. 1','Null')) +
...
theme(legend.position=c(0.9,0.9)) +  background_grid(major = "xy", minor = "y")
#dev.off()
ggp
save_plot("/home/ob219/git/thesis/ch2/pdf/blockshifter_type1_sims.pdf",ggp,base_height=8)
