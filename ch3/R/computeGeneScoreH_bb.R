...
static.args <- list(
int = '/home/ob219/rds/hpc-work/DATA/JAVIERRE_GWAS/RDATA/javierre_interactions.RData',
frags = '/home/ob219/rds/hpc-work/DATA/JAVIERRE_GWAS/RDATA/javierre_frags.by.ld.RData',
csnps = '/home/ob219/rds/hpc-work/DATA/JAVIERRE_GWAS/RDATA/javierre_csnps.by.ld.RData',
target.gene.cSNPs.only = 1,
sets = '/home/ob219/rds/hpc-work/DATA/JAVIERRE_GWAS/support/thesis_tree.yaml',
...
if(FALSE){
  af <- list.files(path="/home/ob219/share/cogs_bb/sCVPP",pattern="*.RDS",full.names=TRUE)
  af <- c(af,list.files(path="/home/ob219/share/Data/GWAS-summary/uk_biobank_neale_summary_stats_2018/cogs_bb_posteriors",pattern="*.RDS",full.names=TRUE))
  cmds <- sapply(af,function(f){
    sprintf("Rscript /home/ob219/git/thesis/computeGeneScoreH_bb.R -p %s -o /home/ob219/share/cogs_bb/COGS",f)
  })
  write(cmds,file="~/tmp/qstuff/cogsh.txt")
}

TEST<-FALSE
option_list = list(
        make_option(c("-p", "--pmi_file"), type="character", default=NULL,
              help="pmi source file", metavar="character"),
        make_option(c("-o", "--out_dir"), type="character", default=NULL,
              help="where to write the results", metavar="character")
)
if(!TEST){
  ...
  args<-list(
    pmi_file = '/home/ob219/share/cogs_bb/sCVPP/UC.RDS',
    out_dir = '/home/ob219/share/cogs_bb/COGS/'
  )


}
args <- c(args,static.args)
#all.thresh<-0
BF.thresh<-3
...
sCVPP.DT <- readRDS(pmi.file)

#test.pmi<-fread(pmi.file,header=TRUE)
pmi.gr<-with(sCVPP.DT,GRanges(seqnames=Rle(chr),ranges=IRanges(start=pos,width=1L),ppi=cvPP,rs=paste(chr,pos,sep=':')))
